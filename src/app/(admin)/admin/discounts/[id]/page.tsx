...
export default function EditDiscountPage({
  ...
  const { id: discountId } = use(params)
  ...
  useEffect(() => {
    async function fetchDiscount() {
      try {
        ...
        const discountData = await pb.collection('discounts').getOne<Discount>(discountId)

        if (!discountData) {
          toast({
            title: 'Discount not found',
            description: 'The requested discount does not exist.',
            variant: 'destructive',
          })
        ...
        // Format expires date for datetime-local input
        let expiresAtFormatted = ''
        if (discountData.expiresAt) {
          const date = new Date(discountData.expiresAt)
          expiresAtFormatted = date.toISOString().slice(0, 16)
        }

        // Reset form with discount data
        reset({
          code: discountData.code,
          type: discountData.type,
          value: discountData.type === 'fixed' ? discountData.value / 100 : discountData.value,
          minOrderValue: discountData.minOrderValue > 0 ? discountData.minOrderValue / 100 : undefined,
          maxUses: discountData.maxUses > 0 ? discountData.maxUses : undefined,
  ...
  const onSubmit = async (data: DiscountFormData) => {
    ...
    try {
      ...
      await pb.collection('discounts').update(discountId, discountData)
  ...
  const handleDelete = async () => {
    ...
    try {
      const pb = getPocketBaseClient()
      await pb.collection('discounts').delete(discountId)
            ...
            <Card>
              ...
              <CardContent className="space-y-2 text-sm text-muted-foreground">
                <div className="flex justify-between">
                  <span>ID:</span>
                  <span className="font-mono text-xs">{discount.id}</span>
                </div>
                <div className="flex justify-between">
                  <span>Used:</span>
                  <span>{discount.usedCount} time(s)</span>
                </div>
                <div className="flex justify-between">
                  <span>Created:</span>
                  <span>{formatDate(discount.created)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Updated:</span>
                  <span>{formatDate(discount.updated)}</span>
                </div>
              </CardContent>
            </Card>
          </div>
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete discount code &quot;{discount.code}&quot;? This action
              cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
